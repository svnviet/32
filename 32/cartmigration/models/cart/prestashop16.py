...
class LeCartPrestashop16(LeBasecart):
	...
	def display_config_source(self):
		...
		config_queries = {
			...
			'reviews': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_module WHERE name LIKE 'revws' AND active = 1"
			},
			'lgcomments': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_module WHERE name LIKE 'lgcomments' AND active = 1"
			},
			'custom_product_tab': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_module WHERE name LIKE 'customproducttabs' AND active = 1"
			},
			'blockwishlist': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_module WHERE name LIKE 'blockwishlist' AND active = 1"
			},
			'productcomments': {
				'type': 'select',
		...
		if (config_data['reviews'] and to_len(config_data['reviews']) > 0):
			self._notice['src']['support']['reviews'] = True
		else:
			self._notice['src']['support']['reviews'] = False
		if (config_data['productcomments'] and to_len(config_data['productcomments']) > 0):
			self._notice['src']['support']['productcomments'] = True
			self._notice['src']['support']['reviews'] = True
		else:
			self._notice['src']['support']['productcomments'] = False
			self._notice['src']['support']['reviews'] = False
		if (config_data['lgcomments'] and to_len(config_data['lgcomments']) > 0):
			self._notice['src']['support']['reviews'] = True
			self._notice['src']['support']['lgcomments'] = True
		else:
			self._notice['src']['support']['reviews'] = False
			self._notice['src']['support']['lgcomments'] = False
		if config_data['custom_product_tab'] and to_len(config_data['custom_product_tab']) > 0:
			self._notice['src']['support']['custom_product_tab'] = True
		else:
			self._notice['src']['support']['custom_product_tab'] = False
		if config_data['blockwishlist'] and to_len(config_data['blockwishlist']) > 0:
			self._notice['src']['support']['blockwishlist'] = True
			self._notice['src']['support']['reviews'] = True
		else:
			self._notice['src']['support']['blockwishlist'] = False
			self._notice['src']['support']['reviews'] = False
		...
		self._notice['src']['config']['downloadable_path'] = '/download'
	...
	def display_config_target(self):
		...
		module_check = ['lecmprepass', 'leurlrewrite', 'productcomments']
		...
		config_queries = {
			...
			'orders_status': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_order_state_lang WHERE id_lang = '" + self._notice['target'][
					'language_default'] + "'",
			},
			'customer_group': {
				'type': 'select',
		...
		if config_data['module'] and to_len(config_data['module']) > 0:
			for module in config_data['module']:
				if module['name'] == 'productcomments':
		...
		self._notice['target']['support']['pages'] = False
	...
	def get_query_display_import_source(self, update = False):
		...
		queries = {
			...
			'categories': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM _DBPRF_category WHERE id_parent != 0 AND is_root_category != 1 AND id_category > 1 AND id_category " + compare_condition + to_str(self._notice['process']['categories']['id_src']),
			},
			'products': {
				'type': 'select',
			...
			'reviews': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM _DBPRF_revws_review WHERE entity_type LIKE 'product' AND id_review " + compare_condition + to_str(self._notice['process']['reviews']['id_src']),
			},
			'coupons': {
				'type': 'select',
		...
		if self._notice['src']['support'].get('lgcomments'):
			queries['reviews'] = {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM _DBPRF_lgcomments_productcomments WHERE id_lgcomments_productcomments " + compare_condition + to_str(self._notice['process']['reviews']['id_src']),
			}
		if self._notice['src']['support'].get('productcomments'):
			queries['reviews'] = {
				'type': 'select',
	...
	def clear_target_products_demo(self):
		...
		for table in tables:
			...
			if table == 'lecm_rewrite':
				where = " WHERE type = 'product' AND id_desc IN " + product_id_con
			clear_table = self.get_connector_data(self.get_connector_url('query'), {
				'query': json.dumps({
					'type': 'query', 'query': "DELETE FROM `_DBPRF_" + table + "`" + where
				})
	...
	def clear_target_reviews_demo(self):
		...
		for table in tables:
			clear_table = self.get_connector_data(self.get_connector_url('query'), {
				'query': json.dumps({
					'type': 'query', 'query': "DELETE FROM `_DBPRF_" + table + "` WHERE review_id IN " + self.list_to_in_condition(review_ids)
				})
			})
			if (not clear_table) or (clear_table['result'] != 'success'):
				self.log("Clear data failed. Error: Could not empty table " + table, 'clear')
	...
	def convert_manufacturer_export(self, manufacturer, manufacturers_ext):
		...
		manufacturer_data['description'] = get_row_value_from_list_by_field(manufacturers_ext['data']['manufacturer_lang'], 'id_lang', self._notice['src']['language_default'], 'description')
		manufacturer_data['thumb_image']['url'] = self.get_url_suffix(self._notice['src']['config']['image_manufacturer']).rstrip('/')
		manufacturer_data['thumb_image']['path'] = 'm/' + to_str(manufacturer['id_manufacturer']) + '.jpg'
		...
		manufacturer_data['meta_title'] = get_row_value_from_list_by_field(manufacturers_ext['data']['manufacturer_lang'], 'id_lang', self._notice['src']['language_default'], 'meta_title')
		manufacturer_data['meta_description'] = get_row_value_from_list_by_field(manufacturers_ext['data']['manufacturer_lang'], 'id_lang', self._notice['src']['language_default'], 'meta_description')
	...
	def manufacturer_import(self, convert, manufacturer, manufacturers_ext):
		...
		id_manufacturer = self.import_manufacturer_data_connector(
			self.create_insert_query_connector('manufacturer', manufacturer_data), True, convert['id'])
		if id_manufacturer:
	...
	def get_categories_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM _DBPRF_category WHERE id_parent != 0 AND is_root_category != 1 AND id_category > 1 AND id_category >" + to_str(id_src) + " ORDER BY id_category ASC LIMIT " + to_str(limit)
		}
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			...
			'check_seo_category': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_configuration WHERE name IN ('PS_ROUTE_category_rule')",
			},
	...
	def convert_category_export(self, category, categories_ext):
		...
		shops = get_list_from_list_by_field(categories_ext['data']['category_shop'], 'id_category', category['id_category'])
		...
		category_data['url_key'] = category_description_def.get('link_rewrite')
		for language_id, language_label in self._notice['src']['languages'].items():
			...
			category_language_data['url_key'] = category_description_lang.get('link_rewrite')
	...
	# 	if (not categories) or (categories['result'] != 'success') or not categories['data']:
	...
	def category_import(self, convert, category, categories_ext):
		...
		id_category = self.import_category_data_connector(self.create_insert_query_connector('category', category_data),
		                                                  True, convert['id'])
		if id_category:
			...
			if self._notice['support']['site_map'] and convert.get('store_ids'):
				...
				for src_store in convert['store_ids']:
					...
					category_lang_data = {
						...
						'name': self.strip_html_tag(category_lang['name']),
						'description': self.strip_html_tag(str(category_lang['description'])),
						'link_rewrite': to_str(category_lang['name'].lower()).replace(' ', '-'),
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'product_attachment': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_product_attachment WHERE id_product IN " + self.list_to_in_condition(product_ids)
			},
			'pack': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_pack WHERE id_product_pack IN " + self.list_to_in_condition(product_ids)
			},
			'check_seo_product': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_configuration WHERE name IN ('PS_ROUTE_product_rule')",
			},
			# 'product_supplier': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM _DBPRF_product_supplier WHERE id_product_attribute != '0' AND id_product IN " + self.list_to_in_condition(product_ids),
			# },
			# 'supplier': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM _DBPRF_supplier ",
			# },
		}

		if self._notice['src']['support'].get('custom_product_tab'):
			product_ext_queries['custom_product_tab'] = {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_customproducttabs_content WHERE product_id IN " + self.list_to_in_condition(product_ids)
			}
		...
		attachment_ids = duplicate_field_value_from_list(product_ext['data']['product_attachment'], 'id_attachment')
		if self._notice['src']['support'].get('custom_product_tab'):
			content_ids = duplicate_field_value_from_list(product_ext['data'].get('custom_product_tab'), 'content_id')
			tab_ids = duplicate_field_value_from_list(product_ext['data'].get('custom_product_tab'), 'tab_id')
		product_ext_rel_queries = {
			...
			'feature_value': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_feature_value  WHERE id_feature IN " + self.list_to_in_condition(feature_ids)
			},
			'feature_value_lang': {
				'type': 'select',
			...
			'attachment': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_attachment WHERE id_attachment IN " + self.list_to_in_condition(attachment_ids)
			},
			'attachment_lang': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_attachment_lang WHERE id_attachment IN " + self.list_to_in_condition(attachment_ids)
			},
			'image_attribute': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_product_attribute_image WHERE id_product_attribute IN " + self.list_to_in_condition(
					product_attribute_ids)
			},
		}
		if self._notice['src']['support'].get('custom_product_tab'):
			product_ext_rel_queries['customproducttabs_lang'] = {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_customproducttabs_lang WHERE tab_id IN " + self.list_to_in_condition(tab_ids)
			}
			product_ext_rel_queries['customproducttabs_content_lang'] = {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_customproducttabs_content_lang WHERE content_id IN " + self.list_to_in_condition(content_ids)
			}
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['weight'] = to_int(product['weight']) if to_int(product['weight']) == to_decimal(product['weight']) else to_decimal(product['weight'])
		product_data['width'] = to_int(product['width']) if to_int(product['width']) == to_decimal(product['width']) else to_decimal(product['width'])
		product_data['height'] = to_int(product['height']) if to_int(product['height']) == to_decimal(product['height']) else to_decimal(product['height'])
		product_data['length'] = to_int(product['depth']) if to_int(product['depth']) == to_decimal(product['depth']) else to_decimal(product['depth'])
		product_data['status'] = True if to_int(product['active']) == 1 else False
		product_data['qty'] = stock_data_main['quantity'] if stock_data_main and stock_data_main['quantity'] else 0
		product_data['manage_stock'] = True if stock_data_main and to_int(stock_data_main['quantity']) > 0 else False
		product_data['created_at'] = convert_format_time(product['date_add'])
		product_data['updated_at'] = convert_format_time(product['date_upd'])
		# supplier_name = get_list_from_list_by_field(products_ext['data']['supplier'], 'id_supplier', product['id_supplier'])
		# if supplier_name:
		# 	for supplier in supplier_name:
		# 		product_data['Suppliers']=supplier['name']
		# suppliers = get_list_from_list_by_field(products_ext['data']['product_supplier'], 'id_product', product['id_product'])
		# if not suppliers:
		# 	product_data['Reference of Supplier']= product['supplier_reference']
		...
		specific_price_sales = get_list_from_list_by_field(specific_price, 'id_group', 0)
		list_special = get_list_from_list_by_field(specific_price_sales, 'from_quantity', 1)
		...
		if special:
			...
			if special_price and to_decimal(product_data['price']) > to_decimal(special_price) > 0:
				product_data['special_price']['price'] = to_decimal(special_price)
		...
		for specific_price_tier in specific_price:
			if to_int(specific_price_tier['id_group']) != 0:
				tier_price_data = self.construct_product_tier_price()
				tier_price_data['id'] = specific_price_tier['id_specific_price']
				tier_price_data['qty'] = specific_price_tier['from_quantity']
				tier_price_data['price'] = specific_price_tier['price'] if specific_price_tier['price'] and to_decimal(specific_price_tier['price']) != 0.000 else (to_decimal(product_data['price']) - to_decimal(specific_price_tier['reduction']))
				tier_price_data['start_date'] = convert_format_time(specific_price_tier['from'])
				tier_price_data['end_date'] = convert_format_time(specific_price_tier['to'])
				tier_price_data['customer_group_id'] = specific_price_tier['id_group']
		...
		product_data['url_key'] = product_description_def.get('link_rewrite')
		product_tags = get_list_from_list_by_field(products_ext['data']['product_tag'], 'id_product',
												   product['id_product'])
		if product_tags:
			tags = list()
		...
		for product_description in product_descriptions:
			...
			tag_data = ''
			if product_tags:
				product_tags = get_list_from_list_by_field(products_ext['data']['product_tag'], 'id_lang', product_description['id_lang'])
				if product_tags:
					tags = list()
					for product_tag in product_tags:
						tag = get_row_from_list_by_field(products_ext['data']['tags'], 'id_tag', product_tag['id_tag'])
						if tag:
							tags.append(tag['name'])
					tag_data = (','.join(tags))[0:254]
			product_language_data['tags'] = tag_data
		...
		if product_image_main:
			product_data['thumb_image']['label'] = product_image_main['legend']
		...
		if product_images:
			for product_image in product_images:
				...
				product_image_data['position'] = product_image['position']
		...
		# grouped product
		if to_int(product['cache_is_pack']) == 1:
			product_data['type'] = 'grouped'

		grouped_parent = get_list_from_list_by_field(products_ext['data']['pack'], 'id_product_pack', product['id_product'])
		grouped_parent_ids = duplicate_field_value_from_list(grouped_parent, 'id_product_item')
		product_data['group_products'] = grouped_parent_ids
		grouped_childs = get_list_from_list_by_field(products_ext['data']['pack'], 'id_product_item', product['id_product'])
		grouped_child_ids = duplicate_field_value_from_list(grouped_childs, 'id_product_pack')
		product_data['parent_grouped'] = grouped_child_ids

		# downloadable
		product_downloadables = get_list_from_list_by_field(products_ext['data']['product_attachment'], 'id_product', product_data['id'])
		if product_downloadables:
			product_data['type'] = 'downloadable'
			for product_downloadable in product_downloadables:
				attachment = get_row_from_list_by_field(products_ext['data']['attachment'], 'id_attachment', product_downloadable['id_attachment'])
				attachment_lang = get_row_from_list_by_field(products_ext['data']['attachment_lang'], 'id_attachment', product_downloadable['id_attachment'])
				product_download_data = self.construct_product_downloadable()
				product_download_data['path'] = attachment.get('file')
				product_download_data['name'] = attachment_lang['name']
				product_download_data['price'] = product_data['price']
			...
			# 	title = dict()
			#
			# 	link['link_url'] = 'prestashop/' + attachment.get('file')
			# 	link['link_type'] = 'file'
			# 	link['is_shareable'] = 1
			# 	link['is_shareable'] = 1
			# 	link['link_file'] = 'prestashop/' + attachment.get('file')
			# 	price['price'] = product_data['price']
			# 	title['title'] = attachment_lang['name']
			#
			# 	temp_link['link'] = link
			# 	temp_link['title'] = title
			# 	temp_link['price'] = price
			# 	downloadable_link.append(temp_link)
			# product_data['downloadable']['link'] = downloadable_link
			# product_data['downloadable']['links_title'] = ''
			# product_data['downloadable']['links_purchased_separately'] = ''
		childen_products = get_list_from_list_by_field(products_ext['data']['product_attribute'], 'id_product', product['id_product'])
		if childen_products and product_data['type'] != 'downloadable' and product_data['type'] != 'grouped':
			# and product_data['type'] != 'downloadable'
			...
			for childen_product in childen_products:
				...
				for childen_option_name in childen_options:
					...
					for attr_lang in attribute_lang:
						product_name_language = dict()
						product_name_language['id_lang'] = attr_lang['id_lang']
						product_data_name = product_data['languages'][attr_lang['id_lang']]['name'] if attr_lang['id_lang'] in product_data['languages'] else ''
						product_name_language['name'] = to_str(product_data_name) + ' - ' + to_str(attr_lang['name'])
						product_name_languages.append(product_name_language)

				childen_data = self.construct_product_child()
				childen_data['id'] = to_str(product_data['id']) + to_str(childen_product['id_product_attribute'])
				childen_data['name'] = childen_name
				childen_data['sku'] = childen_product['reference']
				childen_data['code'] = product_data['id'] + '-' + to_str(childen_data['sku'])
				childen_data['price'] = to_decimal(product['price']) + to_decimal(childen_product['price'])
				childen_data['status'] = True
				childen_data['ean'] = childen_product['ean13']
				childen_data['upc'] = childen_product['upc']
				# utcs = {'ean13', 'upc', 'jan'}
				# for utc in utcs:
				# 	if utc in childen_product:
				# 		utc_import = self.construct_product_attribute()
				# 		utc_import['option_code'] = utc
				# 		utc_import['option_mode'] = 'backend'
				# 		utc_import['option_name'] = utc
				# 		utc_import['option_type'] = 'text'
				# 		utc_import['option_value_code'] = childen_product[utc]
				# 		utc_import['option_value_name'] = childen_product[utc]
				# 		childen_data['attributes'].append(utc_import)
				childen_image = get_list_from_list_by_field(products_ext['data']['image_attribute'], 'id_product_attribute', childen_product['id_product_attribute'])
				if childen_image:
					url_product_image = self.get_url_suffix(self._notice['src']['config']['image_product'])
					childen_data['thumb_image']['url'] = url_product_image
					id_prd = childen_image[0]['id_image']
					childen_data['thumb_image']['path'] = self._get_image_path(id_prd)
				if childen_image:
					for product_child_image in childen_image:
						product_child_image_data = self.construct_product_image()
						product_child_image_data['url'] = self.get_url_suffix(self._notice['src']['config']['image_product'])
						product_child_image_data['path'] = self._get_image_path(product_child_image['id_image'])
						childen_data['images'].append(product_child_image_data)
				if list_special:
					special_childen = get_row_from_list_by_field(list_special, 'id_product_attribute', childen_product['id_product_attribute'])
					if special_childen:
						special = special_childen
				if special:
					if special['reduction_type'] == 'amount':
						special_price = to_decimal(childen_data['price']) - to_decimal(special['reduction'])
					else:
						special_price = to_decimal(childen_data['price']) - to_decimal(to_decimal(special['reduction']) * to_decimal(childen_data['price']))
					if special_price and to_decimal(childen_data['price']) > to_decimal(special_price) > 0:
						childen_data['special_price']['price'] = special_price
						childen_data['special_price']['start_date'] = convert_format_time(special['from'])
						childen_data['special_price']['end_date'] = convert_format_time(special['to'])

				childen_data['weight'] = to_decimal(product['weight']) + to_decimal(childen_product['weight'])
				qty = get_row_value_from_list_by_field(stock_data_all, 'id_product_attribute', childen_product['id_product_attribute'], 'quantity')
				childen_data['qty'] = to_int(qty)
				childen_data['manage_stock'] = True if to_int(qty) > 0 else False
				childen_data['created_at'] = convert_format_time(childen_product['available_date'])
				childen_data['update_at'] = convert_format_time(childen_product['available_date'])

				for product_description in product_descriptions:
					childen_language_data = {}
					childen_language_data['name'] = childen_name
					id_lang = product_description['id_lang']
					if product_name_languages:
						childen_language_data['name'] = get_row_value_from_list_by_field(product_name_languages, 'id_lang', id_lang, 'name')
				...
				for childen_option in childen_options:
					...
					if not attribute_group_lang_def or (not attribute_group_lang_def['public_name'] and not attribute_group_lang_def['name']):
						continue
					childen_product_option_data['option_id'] = childen_option['id_attribute_group']
					childen_product_option_data['option_type'] = 'select'
					childen_product_option_data['option_code'] = to_str(attribute_group_lang_def['name']).lower()
					childen_product_option_data['option_code_save'] = childen_product_option_data['option_code']
					childen_product_option_data['option_name'] = attribute_group_lang_def['public_name'] if attribute_group_lang_def.get('public_name') else attribute_group_lang_def['name']
					childen_product_option_data['position'] = attribute_group_lang_def['position']

					# childen_product_option_data['option_type'] = attribute_group_lang_def['group_type']
					...
					childen_product_option_data['option_value_code'] = to_str(attribue_lang_def.get('name')).lower()
					childen_product_option_data['option_value_code_save'] = childen_product_option_data['option_value_code']
				...
				childen_data['attributes'] = sorted(childen_data['attributes'], key=lambda k: k['position'])
	...
	def product_import(self, convert, product, products_ext):
		...
		product_data = {
			...
			'price': convert['price'],
			'reference': convert['sku'][0:32] if convert['sku'] else '' ,
			'supplier_reference': '',
			'location': '',
		...
		if self._notice['config']['pre_prd']:
			...
			id_product = convert['id']
			params = False
		import_product = self.import_product_data_connector(self.create_insert_query_connector('product', product_data), params, convert['id'])
		if import_product and not id_product:
			id_product = import_product
		if id_product:
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		...
		if convert['type'] == self.PRODUCT_GROUP:
			list_pack = list()
			if 'group_products' in convert and convert['group_products']:
				for child_src in convert['group_products']:
		...
		if 'parent_grouped' in convert:
			for parent_src in convert['parent_grouped']:
				parent_target = self.get_map_field_by_src(self.TYPE_PRODUCT, parent_src['id'])
				if parent_target:
					pack_data = {
						'id_product_pack': parent_target,
						'id_product_item': product_id,
						'id_product_attribute_item': 0,
						'quantity': get_value_by_key_in_dict(parent_src, 'qty', 1),
					}
					pack_query = self.create_insert_query_connector("pack", pack_data)
					all_queries.append(pack_query)
		if self._notice['config']['seo'] or self._notice['config']['seo_301']:
			for seo_url in convert['seo']:
				...
				seourl_query = self.create_insert_query_connector("lecm_rewrite", leurlrewrite)
				all_queries.append(seourl_query)
		all_query = list()
		url_image = self.get_connector_url('image')
		qty = convert['qty']
		if 'manage_stock' in convert and convert['manage_stock'] == True:
			qty = convert['qty']
		else:
			qty = 9999
		stock_available_data = {
			...
			if self._notice['support']['site_map']:
				...
				for src_store in convert['store_ids']:
					...
					product_lang_data = {
						...
						'id_shop': id_shop,
						'id_lang': target_language,
						'name': self.strip_html_tag(to_str(product_lang['name'])[0:128]),
						'description': self.strip_html_tag(to_str(product_lang['description']).replace('\\\\', '\\')),
		...
		if convert['special_price']['price']:
			specific_price_data = {
				...
				'from': convert['special_price']['start_date'] if convert['special_price']['start_date'] else get_current_time(),
				'to': convert['special_price']['end_date'] if convert['special_price']['end_date'] else get_current_time(),
			}
		...
		if convert['tier_prices']:
			for tier_prices in convert['tier_prices']:
				specific_price_data = {
					...
					'reduction_type': 'amount',
					'from': tier_prices['start_date'] if tier_prices['start_date'] else get_current_time(),
					'to': tier_prices['end_date'] if tier_prices['end_date'] else get_current_time(),
				}
				all_query.append(self.create_insert_query_connector('specific_price', specific_price_data))
			...
			for group_price in convert['group_prices']:
				if group_price['price']:
					specific_price_data = {
						...
						'reduction_type': 'amount',
						'from': convert['special_price']['start_date'] if convert['special_price'][
							'start_date'] else get_current_time(),
						'to': convert['special_price']['end_date'] if convert['special_price'][
							'end_date'] else get_current_time(),
					}
					all_query.append(self.create_insert_query_connector('specific_price', specific_price_data))

		if convert['attributes']:
			for key, product_attribute_data in enumerate(convert['attributes']):
				id_feature = self.get_map_field_by_src(self.TYPE_ATTR, product_attribute_data['option_id'],
				                                       product_attribute_data['option_code_save'])
				if not id_feature:
					feature_data = {
						'position': key,
					}
					id_feature = self.import_data_connector(self.create_insert_query_connector('feature', feature_data),
					                                        'product')
					self.insert_map(self.TYPE_ATTR, product_attribute_data['option_id'], id_feature,
					                product_attribute_data['option_code_save'])
					feature_shop_data = {
					...
					if self._notice['support']['site_map'] and 'store_ids' in convert:
						for src_store in convert['store_ids']:
							...
							if src_store in self._notice['map']['site']:
								...
								all_query.append(self.create_insert_query_connector('feature_shop', feature_shop_data))
					for src_language, target_language in self._notice['map']['languages'].items():
						feature_lang = product_attribute_data['option_languages'][src_language] if src_language in \
						                                                                           product_attribute_data[
							                                                                           'option_languages'] and \
						                                                                           product_attribute_data[
							                                                                           'option_languages'][
							                                                                           src_language] else product_attribute_data
						feature_lang_data = {
							'id_feature': id_feature,
							'id_lang': target_language,
				...
				id_feature_value = self.get_map_field_by_src(self.TYPE_ATTR_VALUE,
				                                             product_attribute_data['option_value_id'],
				                                             product_attribute_data['option_value_code_save'])
				if not id_feature_value:
					feature_value_data = {
						'id_feature': id_feature,
						'custom': 0,
					}
					id_feature_value = self.import_data_connector(
						self.create_insert_query_connector('feature_value', feature_value_data), 'product')
					self.insert_map(self.TYPE_ATTR_VALUE, product_attribute_data['option_value_id'], id_feature_value,
					                product_attribute_data['option_value_code_save'])
					for src_language, target_language in self._notice['map']['languages'].items():
						feature_value_lang = product_attribute_data['option_value_languages'][
							src_language] if src_language in product_attribute_data['option_value_languages'] and \
						                     product_attribute_data['option_value_languages'][
							                     src_language] else product_attribute_data
						...
						all_query.append(
							self.create_insert_query_connector('feature_value_lang', feature_value_lang_data))
		...
		if self.get_migrate_product_extend_config():
			...
			if children_list and to_len(children_list) > self.VARIANT_LIMIT:
				...
				children_list = list()
			if children_list:
				for key, children_data in enumerate(convert['children']):
					id_product_attribute = self.get_map_field_by_src(self.TYPE_CHILD, children_data['id'],
					                                                 children_data['code'])
					if not id_product_attribute:
						product_attribute_data = {
							...
							'quantity': children_data['qty'] if 'manage_stock' in children_data and children_data[
								'manage_stock'] == True else 9999,
							'weight': to_decimal(children_data['weight']) - to_decimal(convert['weight']),
							'available_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
						}
						id_product_attribute = self.import_data_connector(
							self.create_insert_query_connector('product_attribute', product_attribute_data), 'product')
						...
						all_query.append(
							self.create_insert_query_connector('product_attribute_shop', product_attribute_shop_data))
						if self._notice['support']['site_map'] and 'store_ids' in convert:
							for src_store in children_data['store_ids']:
								...
								if src_store in self._notice['map']['site']:
									...
									all_query.append(self.create_insert_query_connector('product_attribute_shop',
									                                                    product_attribute_shop_data))
						if children_data['special_price']['price']:
							specific_price_data = {
								...
								'reduction_type': 'amount',
								'from': children_data['special_price']['start_date'] if children_data['special_price'][
									'start_date'] else get_current_time(),
								'to': children_data['special_price']['end_date'] if children_data['special_price'][
									'end_date'] else get_current_time(),
							}
							all_query.append(self.create_insert_query_connector('specific_price', specific_price_data))
						stock_available_data = {
							...
							for src_store in children_data['store_ids']:
								...
								if src_store in self._notice['map']['site']:
									...
									stock_available_data = {
										...
										'id_shop': self._notice['map']['site'][src_store],
										'id_shop_group': 0,
										'quantity': children_data['qty'] if 'manage_stock' in children_data and
										                                    children_data['manage_stock'] == True else 9999,
									}
									all_query.append(
						...
						if children_data['thumb_image']['path'] or children_data['thumb_image']['url']:
							image_process = self.process_image_before_import(children_data['thumb_image']['url'],
							                                                 children_data['thumb_image']['path'])
							id_image = get_row_value_from_list_by_field(image_imported, 'url', image_process['url'],
							                                            'id_image')
							if not id_image:
								image_order += 1
								image_data = {
								...
								id_image = self.import_data_connector(
									self.create_insert_query_connector('image', image_data),
									'product')
								image_imported.append({
									'id_image': id_image,
									'url': image_process['url']
								...
								if self._notice['support']['site_map'] and 'store_ids' in convert:
									for src_store in children_data['store_ids']:
										...
										if src_store in self._notice['map']['site']:
											...
											all_query.append(
												self.create_insert_query_connector('image_shop', image_shop_data))
								image_lang_data = {
									'id_image': id_image,
									'id_lang': self._notice['target']['language_default'],
								}
								...
								for image_resize in resize_images:
									resize_data[image_resize['name']] = {
										...
										'path': image_import_path,
										'params': {
											'desc': self.add_prefix_path(
												self._get_image_path_generate(id_image, image_resize['name']),
							...
							# 		'img': {
							# 			'type': 'download',
							# 			'path': self.add_prefix_path(self._get_image_path(id_image),
							# 			                             self._notice['target']['config']['image_product']),
							# 			'params': {
							# 				'url': image_process['url'],
							# 				'rename': True
							...
							# 		'img_default': {
							# 			'type': 'download',
							# 			'path': self.add_prefix_path(self._get_image_path_default(id_image),
							# 			                             self._notice['target']['config']['image_product']),
							# 			'params': {
							# 				'url': image_process['url'],
							# 				'rename': True
							...
							# 		'img_small': {
							# 			'type': 'download',
							# 			'path': self.add_prefix_path(self._get_image_path_small(id_image),
							# 			                             self._notice['target']['config']['image_product']),
							# 			'params': {
							# 				'url': image_process['url'],
							# 				'rename': True
					...
					if children_data['attributes']:
						for key, childen_product_option_data in enumerate(children_data['attributes']):
							id_attribute_group = self.get_map_field_by_src(self.TYPE_OPTION, childen_product_option_data['option_id'], childen_product_option_data['option_code_save'])
							if not id_attribute_group:
								attribute_group_data = {
									'is_color_group': 1 if childen_product_option_data['option_type'] == 'color' else 0,
									'group_type': childen_product_option_data['option_type'],
									'position': key,
								}
								id_attribute_group = self.import_data_connector(
									self.create_insert_query_connector('attribute_group', attribute_group_data), 'product')
								self.insert_map(self.TYPE_OPTION, childen_product_option_data['option_id'], id_attribute_group, childen_product_option_data['option_code_save'])
								attribute_group_shop_data = {
									'id_attribute_group': id_attribute_group,
									'id_shop': self._notice['target']['shop_default'],
								}
								all_query.append(
									self.create_insert_query_connector('attribute_group_shop', attribute_group_shop_data))
								if self._notice['support']['site_map'] and 'store_ids' in convert:
									for src_store in children_data['store_ids']:
										# for src_store, target_store in self._notice['map']['site']:
										if src_store in self._notice['map']['site']:
								...
								for src_language, target_language in self._notice['map']['languages'].items():
									if childen_product_option_data['option_languages']:
										product_attribute_lang_data = childen_product_option_data['option_languages'][
											src_language] if src_language in childen_product_option_data[
											'option_languages'] and childen_product_option_data['option_languages'][src_language] else childen_product_option_data
									else:
							...
							id_attribute = self.get_map_field_by_src(self.TYPE_OPTION_VALUE, childen_product_option_data['option_value_id'], childen_product_option_data['option_value_code_save'])
							if not id_attribute:
								attribute_data = {
									'id_attribute_group': id_attribute_group,
									'color': '',
								...
								id_attribute = self.import_data_connector(
									self.create_insert_query_connector('attribute', attribute_data), 'product')
								self.insert_map(self.TYPE_OPTION_VALUE, childen_product_option_data['option_value_id'], id_attribute, childen_product_option_data['option_value_code_save'])
								attribute_shop_data = {
								...
								if self._notice['support']['site_map'] and 'store_ids' in convert:
									for src_store in children_data['store_ids']:
										...
										if src_store in self._notice['map']['site']:
											...
											all_query.append(
												self.create_insert_query_connector('attribute_shop', attribute_shop_data))
								...
								for src_language, target_language in self._notice['map']['languages'].items():
									...
									all_query.append(
										self.create_insert_query_connector('attribute_lang', attribute_lang_data))
			...
			if 'group_child_ids' in convert and convert['group_child_ids']:
				for group_prd in convert['group_products']:
					id_prd_group = self.get_map_field_by_src(self.TYPE_PRODUCT, group_prd)
					if id_prd_group:
						pack_data = {
							'id_product_pack': product_id,
			...
			if 'group_parent_ids' in convert and convert['group_parent_ids']:
				for parent_group in convert['parent_grouped']:
		...
		if 'children' in convert['relate'] and convert['relate']['children']:
			for relate_child in convert['relate']['children']:
				if relate_child['type'] == 'relate':
					id_child_relate = self.get_map_field_by_src(self.TYPE_PRODUCT, relate_child['id'],
					                                            relate_child['code'])
					if id_child_relate:
						accessory_data = {
		...
		if 'parent' in convert['relate'] and convert['relate']['parent']:
			for relate_parent in convert['relate']['parent']:
				if relate_parent['type'] == 'relate':
					id_parent_relate = self.get_map_field_by_src(self.TYPE_PRODUCT, relate_parent['id'],
					                                             relate_parent['code'])
					if id_parent_relate:
						accessory_data = {
	...
	def get_customers_main_export(self):
		...
		query = {
			...
			# 'query': "SELECT * FROM _DBPRF_customer WHERE id_customer = 1672 "
		}
	...
	def get_customers_ext_export(self, customers):
		...
		if self._notice['src']['support'].get('blockwishlist'):
			customer_ext_queries['wishlist'] = {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_wishlist WHERE id_customer IN " + self.list_to_in_condition(
					customer_ids)
			}
		...
		wishlist_ids = duplicate_field_value_from_list(customers_ext['data'].get('wishlist'), 'id_wishlist')
		...
		if wishlist_ids:
			customers_ext_rel_queries['wishlist_product'] = {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_wishlist_product WHERE id_wishlist IN " + self.list_to_in_condition(wishlist_ids)
			}
	...
	def convert_customer_export(self, customer, customers_ext):
			...
			for key, address_book in enumerate(address_books):
				if address_book['deleted'] == '0':
					...
					if state:
						...
						state_code = to_str(state['iso_code'])
						address_data['state']['state_code'] = state_code[state_code.rfind('-') + 1:]
		...
		if self._notice['src']['support'].get('blockwishlist'):
			customer_data['wishlist'] = list()
			wishlists = get_list_from_list_by_field(customers_ext['data']['wishlist'], 'id_customer', customer['id_customer'])
			for wishlist in wishlists:
				wishlist_data = dict()
				wishlist_id = wishlist['id_wishlist']
				wishlist_data['id'] = wishlist_id
				wishlist_data['shared'] = ''
				wishlist_data['sharing_code'] = ''
				wishlist_data['updated_at'] = convert_format_time(wishlist['date_upd'])
				wishlist_data['items'] = list()
				wishlist_item = get_list_from_list_by_field(customers_ext['data']['wishlist_product'], 'id_wishlist', wishlist_id)
				for item in wishlist_item:
					item_data = dict()
					item_data['id'] = item['id_wishlist_product']
					item_data['product_id'] = item['id_product']
					item_data['store_id'] = wishlist['id_shop']
					item_data['added_at'] = wishlist['date_upd']
					item_data['description'] = ''
					item_data['qty'] = item['quantity']
					item_data['options'] = list()
					item_option_data = dict()
					item_option_data['product_id'] = item['id_product_attribute']
					item_data['options'].append(item_option_data)
					wishlist_data['items'].append(item_data)
				customer_data['wishlist'].append(wishlist_data)
	...
	def customer_import(self, convert, customer, customers_ext):
		...
		id_customer = self.import_customer_data_connector(self.create_insert_query_connector('customer', customer_data),
		                                                  True, convert['id'])
		if id_customer:
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		if convert['address']:
			for key, address_data in enumerate(convert['address']):
				id_address = self.get_map_field_by_src(self.TYPE_ADDRESS, to_int(address_data['id']), to_int(address_data['code']))
				if not id_address:
					id_address_delivery = self.insert_address(customer_id, address_data)
	...
	def get_orders_main_export(self):
		...
		query = {
			...
			# 'query': "SELECT * FROM _DBPRF_orders WHERE id_order = 1"
		}
	...
	def get_orders_ext_export(self, orders):
		...
		orders_ext_queries = {
			...
			# 'order_carrier': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM _DBPRF_order_carrier WHERE id_order IN " + self.list_to_in_condition(order_ids)
			# },
			# 'carrier': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM _DBPRF_carrier "
			# }
		}
		...
		order_detail_ids = duplicate_field_value_from_list(orders_ext['data']['order_detail'], 'id_order_detail')
		orders_ext_rel_queries = {
			...
			'order_detail_tax': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_order_detail_tax WHERE id_order_detail IN " + self.list_to_in_condition(order_detail_ids)
			},
		}
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_data['shipping']['amount'] = order['total_shipping_tax_incl'] if order['total_shipping'] else order['total_shipping']
		...
		order_data['subtotal']['amount'] = order['total_products_wt']
		...
		if billing_address:
			...
			if billing_state:
				...
				state_code = to_str(billing_state['iso_code'])
				order_billing['state']['state_code'] = state_code[state_code.rfind('-') + 1:]
		...
		if delivery_address:
			...
			if delivery_state:
				...
				state_code = to_str(delivery_state['iso_code'])
				order_delivery['state']['state_code'] = state_code[state_code.rfind('-') + 1:]
		...
		for order_product in order_products:
			...
			if 'reduction_amount' in order_product and to_decimal(order_product['reduction_amount']) > 0:
				order_item['price'] = to_decimal(order_product['product_price']) - to_decimal(order_product['reduction_amount'])
			elif 'reduction_percent' in order_product and to_decimal(order_product['reduction_percent']) > 0:
				order_item['price'] = to_decimal(order_product['product_price']) - (to_decimal(order_product['reduction_amount']) * to_decimal(order_product['product_price']) / 100)
			else:
				order_item['price'] = to_decimal(order_product['product_price'])
			order_item['original_price'] = to_decimal(order_item['price'])
			order_item['tax_amount'] = 0.00
			order_item['tax_percent'] = ''
			products_tax = get_list_from_list_by_field(orders_ext['data']['order_detail_tax'], 'id_order_detail', order_product['id_order_detail'])
			if products_tax and to_len(products_tax) > 0:
				order_item['tax_amount'] = to_decimal(products_tax[0]['unit_amount']) * to_decimal(order_item['qty'])
				order_item['tax_percent'] = 0 if to_decimal(order_item['price']) == 0 else to_decimal(products_tax[0]['unit_amount']) / to_decimal(order_item['price'])
			# order_item['price'] = to_decimal(order_item['price']) + to_decimal(order_item['tax_amount'])
			order_item['price'] = to_decimal(order_product['unit_price_tax_excl']) if order_product['unit_price_tax_excl'] else to_decimal(order_item['price']) + to_decimal(order_item['tax_amount'])
			order_item['discount_amount'] = order_item['discount_amount']
			order_item['discount_percent'] = order_item['discount_percent']
			order_item['subtotal'] = (to_decimal(order_item['price'])) * to_decimal(order_item['qty'])
			order_item['total'] = to_decimal(order_item['subtotal'])
		...
		for customer_m in customer_msg:
			...
			order_history['status'] = customer_m['private']
	...
	def order_import(self, convert, order, orders_ext):
		...
		if id_order:
			self.insert_map(self.TYPE_CUSTOMER, convert['id'], id_order, convert['code'])
		else:
	...
	def after_order_import(self, order_id, convert, order, orders_ext):
		...
		id_shop = self._notice['target']['shop_default']
		if self._notice['support']['site_map'] and 'store_id' in convert and convert['store_id'] in self._notice['support']['site_map']:
			id_shop = self._notice['support']['site_map'][convert['store_id']]
		if convert['items']:
			for key, order_item in enumerate(convert['items']):
				if order_item['product']['id'] or order_item['product']['code']:
					id_product = self.get_map_field_by_src(self.TYPE_PRODUCT, order_item['product']['id'], order_item['product']['code'])
	...
	def get_lgcomment_main_export(self):
		id_src = self._notice['process']['reviews']['id_src']
		limit = self._notice['setting']['reviews']
		query = {
			'type': 'select',
			'query': "SELECT * FROM _DBPRF_lgcomments_productcomments WHERE id_lgcomments_productcomments > " + to_str(id_src) + " ORDER BY id_lgcomments_productcomments ASC LIMIT " + to_str(limit)
		}
	...
	def get_reviews_main_export(self):
		id_src = self._notice['process']['reviews']['id_src']
		limit = self._notice['setting']['reviews']
		if self._notice['src']['support']['productcomments']:
			query = {
				'type': 'select',
			...
			query = {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_revws_review WHERE entity_type LIKE 'product' AND id_review > " + to_str(id_src) + " ORDER BY id_review ASC LIMIT " + to_str(limit)
			}
		# reviews = self.get_connector_data(self.get_connector_url('query'), {'query': json.dumps(query)})
		reviews = self.select_data_connector(query, 'reviews')
		if not reviews or reviews['result'] != 'success':
	...
	def get_reviews_ext_export(self, reviews):
		if self._notice['src']['support']['lgcomments']:
			return response_success()
		if self._notice['src']['support']['productcomments']:
			return response_success()
		review_ids = duplicate_field_value_from_list(reviews['data'], 'id_review')
		reviews_ext_queries = {
			'rating': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_revws_review_grade WHERE id_review IN " + self.list_to_in_condition(review_ids)
			},
		}
		reviews_ext = self.select_multiple_data_connector(reviews_ext_queries, 'reviews')
		if not reviews_ext or reviews_ext['result'] != 'success':
			return response_error('err reviews ext data')

		return reviews_ext

	def convert_lgcomments_export(self, review):
		review_data = self.construct_review()
		review_data['id'] = review['id_lgcomments_productcomments']

		review_data['language_id'] = review['id_lang']
		review_data['product']['id'] = review['id_product']
		review_data['product']['name'] = ''
		review_data['customer']['id'] = review['id_customer']
		# review_data['customer']['name'] = review['display_name']
		review_data['title'] = review['title']
		review_data['content'] = review['comment']
		review_data['status'] = True if review['active'] and review['active'] == '1' else False
		review_data['created_at'] = convert_format_time(review['date'])
		review_data['updated_at'] = convert_format_time(review['date'])

		# rating_data = get_row_from_list_by_field(reviews_ext['data']['rating'], 'id_review', review['id_review'])
		rating = self.construct_review_rating()
		rating['rate_code'] = 'default'
		rating['rate'] = review['stars'] if review and review['stars'] else 0
	...
	def convert_review_export(self, review, reviews_ext):
		if self._notice['src']['support']['lgcomments']:
			return self.convert_lgcomments_export(review)
		if self._notice['src']['support']['productcomments']:
			return self.convert_productcomments_export(review)
		review_data = self.construct_review()
		review_data['id'] = review['id_review']

		review_data['language_id'] = self._notice['src']['language_default']
		review_data['product']['id'] = review['id_entity']
		review_data['product']['name'] = ''
		review_data['customer']['id'] = review['id_customer']
		review_data['customer']['name'] = review['display_name']
		review_data['title'] = review['title']
		review_data['content'] = review['content']
		review_data['status'] = True if review['validated'] and review['validated'] == '1' else False
		review_data['created_at'] = convert_format_time(review['date_add'])
		review_data['updated_at'] = convert_format_time(review['date_upd'])

		rating_data = get_row_from_list_by_field(reviews_ext['data']['rating'], 'id_review', review['id_review'])
		rating = self.construct_review_rating()
		rating['rate_code'] = 'default'
		rating['rate'] = rating_data['grade'] if rating_data and rating_data['grade'] else 0
		review_data['rating'].append(rating)

		return response_success(review_data)

	def convert_productcomments_export(self, review):
		review_data = self.construct_review()
	...
	def get_review_id_import(self, convert, review, reviews_ext):
		if self._notice['src']['support']['lgcomments']:
			return review['id_lgcomments_productcomments']
		elif self._notice['src']['support']['productcomments']:
	...
	def get_pages_ext_export(self, pages):
		...
		page_ext_queries = {
			...
			'cms_shop': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_cms_shop WHERE id_cms IN " + page_id_con
			},
		}
	...
	def convert_page_export(self, page, pages_ext):
		...
		cms_shop = get_list_from_list_by_field(pages_ext['data']['cms_shop'], 'id_cms', page['id_cms'])
		for shop_id in cms_shop:
			page_data['stores'].append(shop_id['id_shop'])
	...
	def convert_coupon_export(self, coupon, coupons_ext):
		...
		cart_rule_langs = get_list_from_list_by_field(coupons_ext['data']['cart_rule_lang'], 'id_cart_rule', coupon['id_cart_rule'])
		for cart_rule_lang in cart_rule_langs:
			coupon_lang_data = self.construct_coupon_language()
			coupon_lang_data['title'] = cart_rule_lang['name']
			coupon_data['languages'][cart_rule_lang['id_lang']] = coupon_lang_data
		coupon_data['title'] = get_row_value_from_list_by_field(cart_rule_langs, 'id_lang', self._notice['src']['language_default'], 'name')
		...
		coupon_data['simple_free_shipping'] = coupon['free_shipping']
	...
	def coupon_import(self, convert, coupon, coupons_ext):
		coupon_data = {
			...
			'quantity_per_user': convert['usage_per_customer'] if convert['usage_per_customer'] != None else 0,
			'priority': 1,
			'partial_use': 1,
	...
	def insert_address(self, customer_id, convert):
		...
		address_data = {
			...
			'address1': convert['address_1'],
			'address2': convert['address_2'],
	...
	def categories_default_seo(self, category, categories_ext):
		...
		if categories_ext['data']['check_seo_category'] and 'value' in categories_ext['data']['check_seo_category'][0] and categories_ext['data']['check_seo_category'][0]['value']:
			value = categories_ext['data']['check_seo_category'][0].get('value')
			if (value.find('id') != -1):
				for seo in cat_desc:
					if seo['id_shop'] == self._notice['src']['shop_default']:
						type_seo = self.SEO_DEFAULT
						seo_cate = self.construct_seo_category()
						seo_cate['request_path'] = seo['id_category'] + '-' + seo['link_rewrite']
	...
	def products_default_seo(self, product, products_ext):
		...
		# product_sku = product['reference'].strip().replace(' ', '-').lower()

		product_categories = get_list_from_list_by_field(products_ext['data']['category_product'], 'id_product', product['id_product'])
		category_ids = duplicate_field_value_from_list(product_categories, 'id_category')
		# category_ids = duplicate_field_value_from_list(product_categories['data'], 'id_category')
		...
		for category_id in category_ids:
			categories_ext_queries = {
				'category_lang': {
					'type': 'select',
					'query': "SELECT * FROM _DBPRF_category_lang WHERE id_category = " + to_str(category_id),
				},
			}
			categories_ext = self.select_multiple_data_connector(categories_ext_queries, 'categories')
			...
			for category_ext in categories_ext['data']['category_lang']:
				if products_ext['data']['check_seo_product'] and 'value' in products_ext['data']['check_seo_product'][0] and products_ext['data']['check_seo_product'][0]['value']:
					value = products_ext['data']['check_seo_product'][0].get('value')
					if (value.find('id') != -1):
						for seo in prd_desc:
							if seo['id_shop'] == self._notice['src']['shop_default']:
								type_seo = self.SEO_DEFAULT
								seo_product = self.construct_seo_product()
								seo_product['request_path'] = category_ext['link_rewrite'].strip() + '/' + product['id_product'] + '-' + seo.get('link_rewrite') + '.html'
