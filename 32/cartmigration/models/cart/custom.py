...
class LeCartCustom(LeBasecart):
	...
	def display_import_source(self):
		...
		queries = {
			...
			'categories': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM  _DBPRF_Categories WHERE id > " + to_str(self._notice['process']['categories']['id_src']),
			},
			'products': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM  _DBPRF_Products WHERE id > " + to_str(self._notice['process']['products']['id_src']),
			},
			'customers': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM  _DBPRF_Customers WHERE id > " + to_str(self._notice['process']['customers']['id_src']),
			},
			'orders': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM  _DBPRF_Orders WHERE  id > " + to_str(self._notice['process']['orders']['id_src']),
			},
			'reviews': {
				'type': 'select',
	...
	def get_products_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM  _DBPRF_Products WHERE id > " + to_str(id_src) + " ORDER BY id ASC LIMIT " + to_str(limit)
		}
	...
	def get_customers_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM  _DBPRF_Customers WHERE id > " + to_str(id_src) + " ORDER BY id ASC LIMIT " + to_str(limit)
		}
	...
	def get_customers_ext_export(self, customers):
		...
		customer_ext_queries = {
			'address_book': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_CustomerAddresses WHERE cust_id IN " + self.list_to_in_condition(customers_ids),
			}
		}
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		customer_data['email'] = customer['pw_email']
		customer_data['password'] = customer['password']
		customer_data['first_name'] = customer['ship_fname']
		customer_data['last_name'] = customer['ship_lname']
		...
		customer_data['telephone'] = customer['bill_phone']
		customer_data['fax'] = customer['bill_fax']
